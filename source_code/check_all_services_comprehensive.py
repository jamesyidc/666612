...
try:
    ...
    if result:
        ...
        delay = (beijing_now.replace(tzinfo=None) - last_time).total_seconds() / 60
        print(f"\nK线指标数据:")
        print(f"  最后更新: {result[0]} (北京时间)")
        print(f"  数据延迟: {delay:.1f} 分钟")
        print(f"  记录总数: {result[1]}")
        if delay <= 10:
            print(f"  状态: ✓ 正常运行")
        else:
            print(f"  状态: ✗ 数据过期 (>{delay:.0f}分钟)")
...
try:
    ...
    if result:
        ...
        print(f"\n支撑/阻力快照:")
        print(f"  快照时间: {snapshot_utc.strftime('%Y-%m-%d %H:%M:%S')} UTC")
        print(f"           {snapshot_beijing.strftime('%Y-%m-%d %H:%M:%S')} 北京时间")
...
try:
    cursor.execute("""
        SELECT snapshot_time, rapid_rise_count, rapid_fall_count, market_status
