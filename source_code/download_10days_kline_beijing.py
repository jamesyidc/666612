...
def save_kline_to_db(conn, symbol, timeframe, kline_data):
    ...
    for candle in kline_data:
        ...
        try:
            ...
            """, (symbol, timeframe, timestamp_beijing, open_price, high_price, 
                  low_price, close_price, volume))
            saved_count += 1
        except Exception as e:
            print(f"  ⚠️ 保存数据失败 (ts={timestamp_beijing}): {e}")
...
def download_history_for_symbol(conn, symbol, timeframe, days=10):
    ...
    print(f"目标时间范围（北京时间）:")
    print(f"  {start_time_beijing.strftime('%Y-%m-%d %H:%M')} ~ {end_time_beijing.strftime('%Y-%m-%d %H:%M')}")
    print(f"目标UTC时间戳: >= {start_ts}")
    ...
    while True:
        ...
        if current_after:
            dt = datetime.fromtimestamp(current_after/1000, tz=timezone.utc).astimezone(BEIJING_TZ)
            print(f"批次 {batch_count}: 获取 {dt.strftime('%Y-%m-%d %H:%M')} (北京时间) 之前的数据...", end=' ')
        else:
            print(f"批次 {batch_count}: 获取最新数据...", end=' ')
        ...
        # 检查最早和最新的数据时间
        oldest_ts_utc = int(kline_data[-1][0])
        newest_ts_utc = int(kline_data[0][0])
        
        oldest_dt = datetime.fromtimestamp(oldest_ts_utc / 1000, tz=timezone.utc).astimezone(BEIJING_TZ)
        newest_dt = datetime.fromtimestamp(newest_ts_utc / 1000, tz=timezone.utc).astimezone(BEIJING_TZ)
        
        print(f"  数据时间（北京）: {oldest_dt.strftime('%Y-%m-%d %H:%M')} ~ {newest_dt.strftime('%Y-%m-%d %H:%M')}")
        
        # 保存到数据库（会自动转换为北京时间戳）
        saved = save_kline_to_db(conn, symbol, timeframe, kline_data)
        total_downloaded += saved
        print(f"  已保存 {saved} 条到数据库（北京时间戳）")
        
        # 如果最早的数据已经达到或超过开始时间，停止
        if oldest_ts_utc <= start_ts:
            print(f"  ✓ 已达到目标时间 {start_time_beijing.strftime('%Y-%m-%d %H:%M')} (北京时间)")
            break
        
        # 更新after参数为当前批次最早的UTC时间戳，用于获取更早的数据
        current_after = oldest_ts_utc
