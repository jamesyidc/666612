...
if positions_result.get('code') == '0' and positions_result.get('data'):
    print(f"  持仓数量: {len(positions_result['data'])} 个")
    for pos in positions_result['data']:
        print(f"\n  币种: {pos.get('instId')}")
        print(f"  持仓方向: {pos.get('posSide')}")
        print(f"  持仓数量: {pos.get('pos')} 张")
        ...
        print(f"  杠杆(lever): {pos.get('lever')}")
else:
    print(f"  返回: {positions_result}")
...
if balance_result.get('code') == '0' and balance_result.get('data'):
    ...
    print("\n" + "=" * 80)
    print("【关键对比】")
    print(f"  持仓API的margin: 9.9973 USDT （包含未实现盈亏）")
    print(f"  余额API的frozenBal: {frozen_bal:.4f} USDT （真实冻结保证金）")
    print(f"  差异: {9.9973 - frozen_bal:.4f} USDT")
    print(f"\n  结论: 系统显示的是持仓API的margin字段")
    print(f"       如果你希望显示真实冻结保证金，我可以修改代码")
else:
    print(f"  返回: {balance_result}")
